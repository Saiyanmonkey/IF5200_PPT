...
class JobstreetFetcher:
    ...
    def get_vacancies_from_company_id(self, company_id: str) -> list[dict]:
        job_list_url = f"{URL_DOMAIN}/id/companies/{company_id}/jobs"
        res = requests.get(job_list_url)
        raw_html = res.text
        soup = BeautifulSoup(raw_html, features="html.parser")
        soup = soup.find(id="company-profile_3_panel")
        if soup is None:
            print("Warning: company-profile_3_panel not found")
            return []
        ...
        soup = access_child(soup, "div", 1)
        if soup is None:
            print("Warning: First chain div not found")
            return []
        ...
        for x in soup.find_all("div", recursive=False):
            ...
            if x is None:
                ...
                result.append({
                    "name": x.text,
                    "url": URL_DOMAIN + x.attrs["href"]
                })

        return result
    
    def get_vacancy_description(self, url: str):
